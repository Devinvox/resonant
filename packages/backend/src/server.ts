...
app.post('/api/llm-proxy/*', async (req, res) => {
  try {
    ...
    if (isGlm) {
      ...
      // History Sanitizer: Strip reasoning blocks generated by other models!
      if (body.messages && Array.isArray(body.messages)) {
        ...
        body.messages = body.messages.map((m: any) => {
          if (Array.isArray(m.content)) {
            ...
            if (m.content.length < originalLen) strippedCount++;

            // Additionally strip <think> XML from any text block
            m.content = m.content.map((block: any) => {
              if (block.type === 'text' && typeof block.text === 'string') {
                const newText = block.text.replace(/<think>[\s\S]*?<\/think>/gi, '');
                if (newText.length < block.text.length) strippedCount++;
                return { ...block, text: newText };
              }
              return block;
            });
            // If message is now empty, add a dummy text element so API doesn't crash
            if (m.content.length === 0) m.content = [{ type: 'text', text: '[Reasoning block removed for model compatibility]' }];
          } else if (typeof m.content === 'string') {
            const newContent = m.content.replace(/<think>[\s\S]*?<\/think>/gi, '');
            if (newContent.length < m.content.length) strippedCount++;
            m.content = newContent;
            if (m.content.trim() === '') m.content = '[Reasoning block removed for model compatibility]';
          }
          return m;
        });
        if (strippedCount > 0) {
          console.log(`[LLM Proxy] Sanitized History: Removed ${strippedCount} reasoning blocks before Anthropic forward.`);
        }
      }
    }
